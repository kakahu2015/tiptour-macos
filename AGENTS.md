...
- **Voice Mode**: Gemini Live only. Single-model realtime WebSocket — bidirectional voice (PCM16 16kHz in, PCM16 24kHz out), vision (JPEG screenshots), text transcription, AND tool calling all in one streaming connection. Two tools exposed: `point_at_element(label, box_2d?)` for single-click asks, `submit_workflow_plan(goal, app, steps)` for action workflows. Workflow steps can include `targetContext` (`visibleElement`, `currentHighlight`, `currentSelection`, `focusedElement`) so Gemini can bind actions to the highlighted/selected/focused target generically instead of smuggling target intent through a click label. Gemini produces workflow plans itself inside its tool call — no separate planner model. API key fetched from Worker's `/gemini-live-key` endpoint. `CompanionManager` constructs the session directly — no protocol indirection (the previous OpenAI Realtime backend + `VoiceBackend` protocol were removed in the simplification refactor).
...
- **Element Pointing**: The LLM calls one of two tools. `ElementResolver` resolves the `label` argument to pixel positions via a two-tier lookup:
    ...
    3. **Raw LLM coordinates from `box_2d`** — Gemini's own spatial grounding. Trusted directly when the model emitted box_2d alongside the label, since both come from the same model decision and don't need cross-checking.
...
| `CompanionPanelView.swift` | ~1059 | SwiftUI panel content. Status header, permissions setup, optional workflow checklist, neko mode toggle, developer section, footer. Dark aesthetic via `DS` design system. |
...
| `ElementResolver.swift` | ~377 | Unified single-entry resolver. Given a label (and optional `box_2d` hint), tries AX tree → browser DOM/CDP coordinates → Gemini's raw `box_2d` coordinates. Always produces a global AppKit point so the overlay can fly the cursor directly. |
